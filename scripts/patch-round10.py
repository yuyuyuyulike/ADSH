...
def save(p, s):
    io.open(R + p, 'w', encoding='utf-8').write(s)
...
# ---------------------------------------------------------------- A. SAF 工具
p = 'ui/WorkspaceActions.kt'
s = load(p)
s = rep(s, '''/** 生成附件封面：图片按 2 的幂下采样解码；非图片返回 null（用文件图标兜底） */''',
'''/**
...
'''    fun closeDrawer() = scope.launch { progress.animateTo(0f, tween(190, easing = FastOutSlowInEasing)) }
    ...
    // 绑定工作区 = 系统文件管理器里选一个文件夹（与导入附件同一套 SAF 流程，不展示文件）
...
'''        // dsh 的 heroWorkspaceRow：输入框「外」左上角的工作区入口（文件夹图标 + 名称 + 倒角）
        // 只在「新对话且还没有对话内容」时出现（dsh 的 hero 阶段），开始对话后自动隐藏
