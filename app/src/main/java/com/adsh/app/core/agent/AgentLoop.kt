...
class AgentLoop(
    ...
    suspend fun recordContext(
        ...
        // 权限预设换了 → 给模型补一条 dsh 式的切换通知（user 消息；UI 按上下文注入行画）。
        // dsh 的对照实现是 user-approval 的 setPolicy："The approval policy changed from \"x\" to
        // \"y\" (changed by the user)."；沙箱模式走同一套，正文只说「从 X 换到 Y」——
        // 策略正文只在系统提示词里现装配一份，这里绝不重复它（第 61/62 轮的去重口径）。
        // 顺序很重要：先按**旧**的 sandbox:policy 行判变化，再把那一行就地更新成新文本。
        val currentMode = policyModeOf(parts.injections.firstOrNull { it.label == "sandbox:policy" }?.text)
        val previousMode = policyModeOf(repository.lastNamed(conversationId, "context", "sandbox:policy")?.content)
        if (currentMode != null && previousMode != null && currentMode != previousMode) {
            repository.addMessage(
                conversationId = conversationId,
                role = "user",
                content = "The DSH file policy changed from \"" + previousMode + "\" to \"" +
                    currentMode + "\" (changed by the user).",
                name = ConversationRepository.SANDBOX_SWITCH,
            )
        }
        parts.injections.forEach { injection ->
            ...
            if (previousRow?.content != injection.text) {
                if (previousRow == null) {
                    ...
                    // 同一个 label 只留一条**当前**快照：内容变了就地更新，不再往时间线上堆旧的一份。
                    // dsh 的 runtime-context 也是「最新的快照取代更早的快照」，旧的不会作为独立节点
                    // 继续生效；第 66 轮以前每换一次权限就多一条 sandbox:policy，界面上越堆越多。
                    repository.setMessageContent(previousRow.id, injection.text)
                }
            }
        }
    }
    ...
    /**
     * 从 sandbox:policy 的正文里取模式名。文案是我们自己装配的稳定文本
     * （\"Current DSH file policy: <mode>.\"），只用来判断「换没换」，不做别的用途。
     */
    private fun policyModeOf(text: String?): String? =
