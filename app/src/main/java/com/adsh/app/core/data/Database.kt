...
data class MessageEntity(
    ...
    /**
     * 工具行上携带的图片（JSON 数组，见 ToolImage）—— dsh 的工具输出内容块里的 image 块。
     * 目前只有 read_image（PTC 里作为 run_code 的子调用）会产出；装配请求时按 dsh 的
     * tools-ptc deferContext 作为一条 user 消息回灌模型。
     */
...
interface MessageDao {
    ...
    @Query(
        "SELECT id, conversationId, role, substr(content, 1, 32768) AS content, " +
            ...
            "substr(attachmentsJson, 1, 8192) AS attachmentsJson, " +
            "substr(imagesJson, 1, 8192) AS imagesJson, createdAt " +
    ...
    /**
     * 最后一条**开启轮次**的用户消息（`name IS NULL`）。
     *
     * 插话（[ConversationRepository.STEERING]）与权限切换通知（SANDBOX_SWITCH）都带 name，
     * 它们不是轮次的开头：用量要写在真正的轮首那一行上（轮尾的用量按钮读的就是它）。
     */
    @Query(
        "SELECT * FROM messages WHERE conversationId = :conversationId AND role = :role " +
            "AND name IS NULL ORDER BY id DESC LIMIT 1"
    )
    suspend fun lastTurnOpener(conversationId: Long, role: String): MessageEntity?

    /** 去掉一条消息的 name 标记（插话没人接、成为下一轮的开头时用） */
    @Query("UPDATE messages SET name = NULL WHERE id = :id")
    suspend fun clearName(id: Long)
}
...
abstract class AdshDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         */
        /** v12 → v13：工具行上的图片（dsh 的 read_image 输出内容块） */
        ...
        /**
         * v13 → v14：**同会话目标**的 schema（dsh 的 goal 域 + `messages.goalRound`）。
         *
         * 第 79 轮加过、第 80 轮随功能一起回退 —— 但**这一条迁移必须留着**：
         * 还在 v13 的旧安装（第 78 轮那份 release 及更早）升到 v15 时，Room 得能凑出一条
         * 13 → … → 15 的路径；少了它就会落到下面的 `fallbackToDestructiveMigration`，
         * 把用户的会话整个清空。它建出来的表与列，紧接着被 [MIGRATION_14_15] 摘掉。
         */
            ...
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS goals (" +
                        ...
                        "blockedCode TEXT, " +
                        "blockedMessage TEXT, " +
                        "maxGoalRounds INTEGER NOT NULL, " +
                        "roundsStarted INTEGER NOT NULL, " +
                        "revision INTEGER NOT NULL, " +
                        "createdAt INTEGER NOT NULL, " +
                        "updatedAt INTEGER NOT NULL)"
                )
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_goals_conversationId ON goals (conversationId)")
                db.execSQL("ALTER TABLE messages ADD COLUMN goalRound INTEGER")
            }
        }

        /**
         * v14 → v15：目标功能回退（第 80 轮），把 schema **精确还原成 v13 的形状**。
         *
         * 为什么非要往前升一版：手机上的库在第 79 轮已经升到 v14（多了 `goals` 表与
         * `messages.goalRound`）。直接装回 v13 的代码会让 Room 撞上「降级」——
         * 要么抛异常（打不开会话），要么按 `fallbackToDestructiveMigration` 清库（丢数据），
         * 两种都不能接受。v13 也没有「把列摘掉」的工具（老 SQLite 没有 DROP COLUMN），
         * 所以按 v11 → v12 的老办法重建 messages 再搬数据；迁完 identity hash 与 v13 的
         * schema 完全一致，用户的会话、消息一条不动。
         */
        private val MIGRATION_14_15 = object : androidx.room.migration.Migration(14, 15) {
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                db.execSQL("DROP TABLE IF EXISTS goals")
                val columns = "id, conversationId, role, content, reasoning, toolCallsJson, toolCallId, name, " +
                    "subCallsJson, isError, durationMs, usageJson, attachmentsJson, imagesJson, createdAt"
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS messages_new (" +
                        "id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                        "conversationId INTEGER NOT NULL, role TEXT NOT NULL, content TEXT NOT NULL, " +
                        "reasoning TEXT, toolCallsJson TEXT, toolCallId TEXT, name TEXT, subCallsJson TEXT, " +
                        "isError INTEGER NOT NULL, durationMs INTEGER NOT NULL, usageJson TEXT, " +
                        "attachmentsJson TEXT, imagesJson TEXT, createdAt INTEGER NOT NULL)"
                )
                db.execSQL("INSERT INTO messages_new (" + columns + ") SELECT " + columns + " FROM messages")
...
class ConversationRepository(private val db: AdshDatabase) {
    ...
    companion object {
        ...
        /**
         * 权限预设切换的通知消息标记（第 66 轮）。dsh 的做法：策略变化时给模型追加一条 user 消息
         * `The approval policy changed from "x" to "y" (changed by the user).`；沙箱模式这边同理，
         * 只是正文换成 DSH file policy。它不是「用户的发言」，UI 按上下文注入行渲染（见 TurnList）。
         */
        const val SANDBOX_SWITCH = "sandbox:switch"

        /**
         * 插话发送进来的用户消息标记（dsh 的 `steering` 节点）。
         *
         * dsh 里这条消息由 **next-step inbox 认领**进来的，节点 kind 因此是 `steering` 而不是
         * `user`：它**不开一轮新的**，而是落在正在跑的那一轮里（气泡夹在过程行之间），
         * 那一轮的 turn 状态不变。ADSH 的「插话发送」就是这条路径，所以落库时打上这个标记：
         *  - UI（TurnList.buildChatItems）据此把它排进当前轮的过程条目；
         *  - 轮次计数（AgentLoop / 统计）按 `name == null` 数，插话不算新的一轮。
         *
         * 例外：插话到达时这一轮已经收尾（模型没机会看到它），它就成了**下一轮的开头**，
         * 那时由 `continueIfDangling` 把这个标记去掉（clearName），与 dsh 的 inbox 认领语义一致。
         */
    ...
    /**
     * 最后一条开启轮次的用户消息（`name IS NULL`）—— 轮次计数的口径与 AgentLoop 一致：
     * 带 name 的行是系统通知（SANDBOX_SWITCH）或插话（STEERING），都不开新的一轮。
     */
    suspend fun lastTurnOpener(conversationId: Long, role: String = "user"): MessageEntity? =
        db.messages().lastTurnOpener(conversationId, role)

    /**
     * 去掉一条消息的 name 标记。
     *
     * 唯一的调用点是「插话到达时这一轮已经收尾」（ChatViewModel.continueIfDangling）：
     * 那条消息没被正在跑的那一轮认领，而是成了下一轮的开头 —— 与 dsh 的 inbox 认领语义一致
     * （没被认领就不是 steering 节点，而是普通的 user 节点）。
     */
    suspend fun clearName(messageId: Long) = db.messages().clearName(messageId)
