...
sealed interface WorkspaceHealth {
    data object Active : WorkspaceHealth
    data class NeedsRebind(val reason: String) : WorkspaceHealth
}
...
class WorkspaceManager(
    ...
    fun revalidate(): WorkspaceHealth {
        val workspace = current()
        if (workspace.form == WorkspaceForm.PRIVATE) return WorkspaceHealth.Active
        if (!workspace.root.isDirectory) {
            return WorkspaceHealth.NeedsRebind("目录已不存在：" + workspace.root.absolutePath)
        }
        val probe = "test -w \"" + workspace.root.absolutePath + "\" && echo ADSH_WRITABLE"
        val result = runtime.run(probe, workspaceRoot = workspace.root, timeoutMs = 15_000)
        return if (result.output.contains("ADSH_WRITABLE")) {
            WorkspaceHealth.Active
        } else {
            WorkspaceHealth.NeedsRebind("已失去写入权限：" + workspace.root.absolutePath)
        }
    }
