 ...
 *
 * 设计依据（M0 与第 44 轮真机实证，见 docs/bootstrap-spike.md 与 docs/UI-v6-report.md）：
...
class TermuxRuntime(private val context: Context) {
    ...
    fun environment(workspaceRoot: File?): Array<String> {
        ...
        // LD_PRELOAD：termux-exec + 写围栏 shim（顺序与理由见 preloadValue）。围栏本身还要
        // [fenceEnv] 给了白名单才真的生效。
    ...
    /**
     * 写围栏（dsh 的 fs-sandbox fence）的环境变量：**只有 workspace-write 才挂**。
     *
     ...
     *
     * @param roots 可写白名单（对齐 dsh 的 writableRoots：工作区 + 临时目录）
     * @param mark 哨兵文件：shim 一加载就写它。App 侧用它确认 LD_PRELOAD 真的生效
     *  （拿不到就说明这次运行里围栏没起作用，只能记日志 —— 不能因为「围栏没生效」
     *  就把命令整个拒掉，那等于 workspace-write 下 bash 全不可用）
     */
    fun fenceEnv(mode: String, roots: List<File?>, mark: File?): Array<String> {
        if (mode != com.adsh.app.core.tools.Escalation.WORKSPACE_WRITE) return emptyArray()
        ...
        val whitelist = roots.filterNotNull()
            ...
            .distinct()
        if (whitelist.isEmpty()) return emptyArray()
     ...
     */
    fun shellLaunch(extraEnv: Array<String> = emptyArray()): ShellLaunch = ShellLaunch(
        command = bashPath,
        // -i：交互但**不是** login shell（不 source /etc/profile，理由见 preloadValue 的注释）。
        // PS1 由终端页通过环境变量给，不依赖 profile。
        args = listOf("-i"),
        env = environment(null).toList() + extraEnv.toList(),
