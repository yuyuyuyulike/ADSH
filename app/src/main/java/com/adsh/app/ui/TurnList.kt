...
fun buildChatItems(
    ...
    val open = turn
    if (open != null) {
        if (liveTurnId != null && open.turnKey == liveTurnId) {
            open.addLive(
                streaming = streaming,
                reasoning = reasoning,
                liveCalls = liveCalls,
...
private class TurnBuilder(val key: Long, val startedAt: Long, val usage: TurnUsage? = null) {
    ...
    fun addLive(
        ...
        // 第 81 轮：与**刚落库的这一步**去重。
        //
        // 一步的正文/思考在库里已经有一行（AgentLoop 先落库、再发 StepCommitted），而流式状态
        // 的清零要慢半拍（采样节拍最多 33ms）。这几帧里同一步的正文会同时来自两处：库里那一行
        // + 这段还没清掉的流式尾巴 —— 多出来的一行会把下面的工具行顶下去再弹回来，正是用户看到的
        // 「模型在工具调用之间说话时，工具调用的展示闪烁/撕裂」。
        //
        // 判据只看**最后两条**条目：同一步落库后的形状是 [.., Reasoning, Text]（思考在前、正文在后），
        // 于是思考要能在倒数第二条、正文要在最后一条里找到。只看末尾两条也避免了误伤
        // 「上一步说过同样的话、这一步又说一遍」（中间隔着工具行，不在末尾两条里）。
        if (reasoning.isNotBlank() && !entries.takeLast(2).any {
                it is ProcessEntry.Reasoning && it.text == reasoning
            }
        ) {
        ...
        if (streaming.isNotBlank() && !entries.takeLast(2).any {
                it is ProcessEntry.Text && it.text == streaming
            }
        ) {
